...
def output_root(source_z: int, subdivision: int) -> Path:
...
def build_near(x: int, y: int, overwrite: bool, source_z: int, subdivision: int) -> None:
    ...
    root = output_root(source_z, subdivision)
    ...
    if (
        ...
        print("skip near", stem, f"z={source_z}", f"subdivision={subdivision}")
        return
...
def main() -> None:
    ...
    build_near(args.tile[0], args.tile[1], args.overwrite, args.source_zoom, args.subdivision)
